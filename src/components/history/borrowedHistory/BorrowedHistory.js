...
import classes from "./borrowedHistory.module.scss";
import { useDispatch } from "react-redux";
import { undoTransaction } from "actions";

const { history_table, undo_transaction } = classes;

const BorrowedHistory = ({ historyData }) => {
  const borrowedHistory = [...historyData].reverse();
  ...
  return (
    <table className={history_table}>
      ...
      <tbody>
        {borrowedHistory.map((data, index) => (
          <tr
            style={{
              fontWeight: "300",
              fontSize: "12px",
